...
- Web Push needs **VAPID keys** generated and stored as env vars (`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`). Generate with `npx web-push generate-vapid-keys`.
- New tables to wire up (already in schema): `nudges`, `push_subscriptions` (both exist per the initial migration).
- New UI: a settings page or inline opt-in CTA to register a service worker subscription (`/sw.js` already generated by `@serwist/next`). Persist subscription to `push_subscriptions`.
...
1. `phase-2b-vapid-and-subscriptions` — VAPID setup, service-worker push registration UI, `push_subscriptions` writes, manual "send test push" button gated on signed-in owner.
